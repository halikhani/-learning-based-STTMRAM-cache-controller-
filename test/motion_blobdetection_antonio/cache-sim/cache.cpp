...
Cache::Cache(ll cacheSize, ll blockSize, ll setAssociativity, int level, std::string policy){
    ...
    //TODO: added to init the cache data structure. otherwise it sets everything to 0 (by chance)
...
long long Cache::getCacheBlockTag(long long blockPosition){
    return this->cacheBlocks[blockPosition];
}


long long Cache::getOffsetSize(){
    return this->offsetSize;
}

long long Cache::getIndexSize(){
    return this->indexSize;
}

long long Cache::getTagSize(){
    return sizeof(long long) * 8 - this->offsetSize - this->indexSize; //TODO there ws a bug here. I added *8
}

long long Cache::getBlockSize(){
