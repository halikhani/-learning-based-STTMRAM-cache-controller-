...
int main(int argc, char **argv) {
  ...
  if(argc<4 || argc>6) {
    ...
    std::cout << "This application has 7 different data structures so it requires 7 different QLs."  << std::endl;
...
int** blob_detection(sImage* originalImage, sImage* blobImage, int* num_of_bbs, bool output) {
  ...
  for(r=0, count=1; r<blobImage->rows; r++){
    for (c=0; c<blobImage->cols; c++){
      ...
      //we_assign_uchar(blobImage->data + r*blobImage->cols+c, (char)value, BLOB_IMAGE);
    }
  }
...
///
//this is a very ugly trick. Actually in the previous loop each block of BLOB_IMAGE is assigned ad subsequently read several times; the block stays in L1 and no write error occurs
//however, in this second loop all blocks in the data structure causes an L1 miss, at the end of the day the entire data structure goes back in L2 from L1 and returns in L1.
//subsequent 5 lines of code forces write errors in each block when it is loaded back in L1 (actually the error occurred before in the time but it does not matter...
      cm.getLastAccessStats(1, &miss, &newBlockBaseAddress, &oldBlockBaseAddress);
      oldBlockBaseAddress = -1; //random block...
      if(miss){
        we_miss_block((long long) (blobImage->data), blobImage->size*sizeof(unsigned char), newBlockBaseAddress, oldBlockBaseAddress, blockSize, BLOB_IMAGE);
      }
///
