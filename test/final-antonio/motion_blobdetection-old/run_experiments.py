...
if __name__ == "__main__":
  ...
  compile_cmd = ['g++', 'motion_sttmram.cpp', 'cache-sim/cache.cpp', 'cache-sim/cache_model.cpp', 'cache-sim/fifo.cpp', 'cache-sim/lfu.cpp', 'cache-sim/lru.cpp', 
      'cache-sim/nru.cpp', 'cache-sim/plru.cpp', 'cache-sim/srrip.cpp', '-std=c++11', '-O3', '-lm', '-o', MOTION_STTMRAM_CMD]
  ...
  if ret!=0:
    ...
    print errors2
    sys.exit(0)
  gpp_p = subprocess.Popen(['g++', 'motion.cpp', '-std=c++11','-O3', '-lm', '-o', MOTION_CMD], stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
  errors = gpp_p.stdout.read()
  errors2 = gpp_p.stderr.read()
  ret= gpp_p.wait()
  if ret!=0:
